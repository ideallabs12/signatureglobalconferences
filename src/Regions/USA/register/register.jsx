          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((c) => (
              <div
                ...
                className={`usa-rg-country-option${c.code === value ? " usa-rg-country-option--active" : ""}`}
                onMouseDown={(e) => { e.preventDefault(); handleSelect(c); }}
                role="option"
                aria-selected={c.code === value}
              >
                <span className="usa-rg-country-option__code">{c.code}</span>
                <span className="usa-rg-country-option__name">{c.name}</span>
              </div>
            ))
          )}
        </div>
    ...
    <div className="usa-rg-step-indicator" role="navigation" aria-label="Registration steps">
      {steps.map((s, i) => (
        <div key={s.num} className="usa-rg-step-indicator__item">
          <div
            className={`usa-rg-step-indicator__circle ${currentStep > s.num ? "done" : currentStep === s.num ? "active" : ""}`}
  ...
  return (
    <section className="usa-rg-hero" aria-label="Registration hero">
      ...
      <div className="usa-rg-hero__content">
        ...
        <h1 className="usa-rg-hero__title">Claim Your <em>Global Stage</em></h1>
...
function Step1({ fields, errors, set, setField, allConferences }) {
  ...
  return (
    <div className="usa-rg-form-body">
      <div className="usa-rg-divider">Personal Information</div>
...
function Step2({ fields, errors, setField, allPackages, companionPrice, extraNightPrice }) {
  ...
  return (
    <div className="usa-rg-form-body">
      <div className="usa-rg-divider">Participation Type</div>

      {errors.speakerType && (
        <span className="usa-rg-error-msg" role="alert" style={{ marginBottom: 10, display: "block" }}>
          {errors.speakerType}
        </span>
      )}

      <div className="usa-rg-type-toggle" role="radiogroup" aria-label="Participation type">
        {[
          ...
          <button
            key={type}
            type="button"
            className={`usa-rg-type-btn${fields.speakerType === type ? ` usa-rg-type-btn--active${type === "virtual" ? " usa-rg-type-btn--virtual" : ""}` : ""}`}
      ...
      {fields.speakerType && (
        <>
          <div className="usa-rg-divider">Choose Your Package</div>
          {errors.packageId && (
            <span className="usa-rg-error-msg" role="alert" style={{ marginBottom: 10, display: "block" }}>
              {errors.packageId}
            </span>
      ...
      {fields.speakerType === "physical" && (
        <>
          <div className="usa-rg-divider">Additional Attendees &amp; Nights</div>
          {[
            ...
            <div key={key} className="usa-rg-companion-box">
              <div className="usa-rg-companion-box__info">
                ...
                <div className="usa-rg-companion-box__sub">{sub}</div>
              </div>
...
function Step3({ fields, allConferences, allPackages, companionPrice, extraNightPrice, onEditStep, setField }) {
  ...
  const ReviewSection = ({ title, step, children }) => (
    <div className="usa-rg-review-section">
      <div className="usa-rg-review-section__head">
        ...
        <button className="usa-rg-edit-btn" onClick={() => onEditStep(step)} type="button" aria-label={`Edit ${title}`}>
          Edit
        </button>
      </div>
      {children}
  ...
  return (
    <div className="usa-rg-form-body">
      ...
      <div className="usa-rg-total-bar" aria-label="Final amount">
        <div className="usa-rg-total-bar__left">
  ...
  return (
    <section className="usa-rg-result usa-rg-result--success" role="main" aria-label="Registration successful">
      <div className="usa-rg-result__card">
        <div className="usa-rg-result__icon" aria-hidden="true">
...
function RegistrationForm({ onSuccess, onFail, allConferences, allPackages, companionPrice, extraNightPrice, preselectedConferenceId }) {
  ...
  useEffect(() => {
    if (preselectedConferenceId) {
      ...
      // Always pre-fill region since this register page is USA-only
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      await submitRegistration(fields, allConferences, allPackages, companionPrice, extraNightPrice, "usa");
      onSuccess(fields);
    } catch (err) {
...
export default function Register() {
  ...
  // Pre-selected conference from navigation state (e.g. from event card "Register" button)
        ...
        <>
          ...
          <section className="usa-rg-section" aria-label="Registration form">
            <div className="usa-rg-section__inner">
              {dataLoading ? (
                <div className="usa-rg-loading" aria-live="polite">
                  <span className="usa-rg-spinner" aria-hidden="true" />
